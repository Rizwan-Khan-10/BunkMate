  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] p-4 bg-blue-50 dark:bg-slate-900 text-gray-900 dark:text-white">
      <div className="max-w-5xl mx-auto">
        <h2 className="text-2xl font-bold mb-6 text-center">Attendance Calendar</h2>
          ...
          <div className="mt-8 bg-white dark:bg-slate-950 p-6 rounded-xl shadow-md">
            ...
            {!editing && customSlots.length > 0 && (
              <div className="flex gap-4 mb-4">
                <button
                  ...
                  className={`px-3 py-1 rounded text-white ${markType === 'Present' ? 'bg-green-300 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'}`}
                >
                  Mark All Present
                </button>
                <button
                  ...
                  className={`px-3 py-1 rounded text-white ${markType === 'Absent' ? 'bg-red-300 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700'}`}
                >
                  Mark All Absent
                </button>
              </div>
            ...
            {!editing ? (
              customSlots.length === 0 ? (
                <p className="text-red-500">No timetable available for this day.</p>
              ) : (
                <>
                    ...
                    {customSlots.map((slot, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex justify-between items-center border px-4 py-3 rounded-lg dark:border-gray-700">
                          <div>
                            <div className="font-semibold">{subject?.name || 'Unknown'}</div>
              ...
              <>
                <h4 className="text-lg font-semibold mb-3">Edit Timetable</h4>

                {customSlots.map((slot, index) => (
                  <div key={index} className="flex flex-wrap md:flex-nowrap items-center gap-2 mb-3">
                    <select
                      id={`subject-${index}`}
                      autoComplete="off"
                    ...
                    <input
                      ...
                      className="border px-2 py-1 rounded w-[100px] dark:bg-gray-700 dark:text-white"
                    />
